...
public class AnimationUtil {
    ...
    public static ChangeListener<Number> numberChangeListener(Labeled labeled) {
        return (observable, oldValue, newValue) -> {
            new RubberBand(labeled).setResetOnFinished(true).play();
            if (newValue.intValue() < oldValue.intValue())
                AnimationUtil.getTextGlowRed(labeled).play();
            if (newValue.intValue() > oldValue.intValue())
                AnimationUtil.getTextGlowGreen(labeled).play();
            labeled.setText(newValue + "");
        };
    }
