...
public class UseHeroPower implements Command {
    private final int sourceId;
    private int targetId = 0;
    private final long token;

    public UseHeroPower(HeroObject source, GameObject target, long token) {
    ...
    public Packet visit(SocketWorker worker, IGameServer gameServer, long pid) {
        ...
        return new Packet(controller
                .useHeroPower((HeroObject) pool.getGameObjectById(sourceId)
